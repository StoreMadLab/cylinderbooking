                ...
                <table class="table table-bordered table-striped">
                    ...
                    <?php 
                            ...
                            if(isset($_GET['search']))
                            {
                                ...
                                $query = "SELECT * FROM myorderview WHERE CONCAT(cname,phone,Ddate,dname,m_no) LIKE '%$filtervalues%' ";
                                $query_run = mysqli_query($con, $query);

                                if(mysqli_num_rows($query_run) > 0)
                                {
                                    foreach($query_run as $cylinder)
                                    {
                                        ?>
                                        <tr>
                                            <td><?= $cylinder['cname']; ?></td>
                                            <td><?= $cylinder['adress']; ?></td>
                                            <td><?= $cylinder['phone']; ?></td>
                                            <td><?= $cylinder['no_of_cylinder']; ?></td>
                                            <td><?= $cylinder['T_amount']; ?></td>
                                            <td><?= $cylinder['Ddate']; ?></td>
                                            <td><?= $cylinder['dname']; ?></td>
                                            <td><?= $cylinder['m_no']; ?></td>
                                    </tr>
                                        <?php
